...
/*
   Realtime può consegnare uno snapshot senza tutte le colonne della riga.
   Durante una partita non dobbiamo mai sostituire uno state valido con un
   fallback privo di game_state: quello era il percorso che riportava la UI
   alla schermata "Preparazione della partita...".
*/
function mergeUnoRoomSnapshot(snapshot) {
    ...
    const previousIsPlaying =
        previousRoom.status === "playing" ||
        previousRoom.game_state?.status === "playing";

    /* Un update incompleto/stale non può riportare una partita attiva in lobby. */
    if (previousIsPlaying && snapshot.status === "waiting") {
        return previousRoom;
    }

    const hasGameState = Object.prototype.hasOwnProperty.call(snapshot, "game_state");
    const incomingGameState = snapshot.game_state;
    const keepPreviousGameState =
        previousIsPlaying &&
        snapshot.status === "playing" &&
        (!hasGameState || !incomingGameState);

    currentRoom = {
        ...previousRoom,
        ...snapshot,
        ...(keepPreviousGameState ? { game_state: previousRoom.game_state } : {})
    };
...
async function showGameScreen() {
    ...
    if (currentRoom?.game_state) {
        ...
        /* Stato ammesso solo durante un setup reale, mai come fallback di un
           update Realtime incompleto durante PLAYING. */
        if (gameTurnInfo) gameTurnInfo.textContent = "Partita iniziata";
        if (turnMessage) turnMessage.textContent = "Preparazione della partita...";
    }
        ...
        supabaseClient
            ...
            .on(
                ...
                async payload => {
                    ...
                    const room = mergeUnoRoomSnapshot(payload?.new);
                    if (!room) return;
