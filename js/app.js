...
let cuginiPartyGameReleaseTimer = null;
...
function applyCuginiPartyAdminSettings(
    ...
    if (totalBlock) {
        ...
        const closeButton = document.getElementById("cuginiparty-admin-global-close");
...
const CUGINIPARTY_ADDITION_ORDER = [
    "tris", "uno", "racing", "memory", "pizza", "balloons", "aquarium", "undercover", "block-blast", "tetris",
    "wordle", "reaction-time", "color-match", "hangman", "snake", "sliding-puzzle", "number-merge", "water-sort", "darts",
    "cube-rush", "polizia-ladri", "indovina-chi", "gatto-topo"
];
const CUGINIPARTY_DEFAULT_DESCRIPTIONS = {
    tris: "Sfida un amico o affronta il computer nel classico gioco del Tris.",
    uno: "Gioca a UNO con i tuoi amici online e sfidali in stanze private.",
    racing: "Corri, schiva gli ostacoli e raggiungi il traguardo nel minor tempo possibile.",
    memory: "Trova tutte le coppie di carte nel minor numero di mosse possibile.",
    pizza: "Prepara gli ordini dei clienti, cucina le pizze e guadagna!",
    balloons: "Scoppia più palloncini possibile prima che il tempo finisca.",
    aquarium: "Esplora l'acquario, raccogli stelle magiche e trova tesori nascosti.",
    undercover: "Scopri chi ha una parola diversa, sullo stesso telefono oppure online.",
    "block-blast": "Posiziona i blocchi, completa righe e colonne e cerca di ottenere il punteggio più alto.",
    tetris: "Incastra i tetramini, completa le righe e resisti mentre la velocità aumenta.",
    wordle: "Indovina la parola italiana di cinque lettere in sei tentativi.",
    "reaction-time": "Attendi il segnale, poi reagisci più velocemente possibile.",
    "color-match": "Sfida l'effetto Stroop: scegli il colore del testo, non la parola.",
    hangman: "Scopri la parola prima che il disegno dell'impiccato sia completo.",
    snake: "Mangia il cibo, cresci e supera il tuo record senza colpire il corpo.",
    "sliding-puzzle": "Riordina le tessere numerate scegliendo la difficoltà che preferisci.",
    "number-merge": "Unisci le tessere uguali e prova a creare il 2048.",
    "water-sort": "Ordina l'acqua colorata finché ogni provetta contiene un solo colore.",
    darts: "Mira, lancia e chiudi la partita nel classico gioco delle freccette.",
    "cube-rush": "Corri, salta e cambia forma attraverso tre livelli costruiti intorno al ritmo.",
    "indovina-chi": "Fai domande, elimina i sospetti e scopri il personaggio segreto del tuo avversario prima che lui scopra il tuo.",
    "gatto-topo": "Tocca al momento giusto, fai scattare la zampa e prova a catturare il topo prima che scappi."
};
const CUGINIPARTY_DEFAULT_DISPLAY_META = {
    tris: { icon: "assets/tris-icon.png", category: "Party" },
    uno: { icon: "assets/uno-icon.png", category: "Party" },
    racing: { icon: "assets/racing-2d-icon.png", category: "Arcade" },
    memory: { icon: "assets/memory-icon.png", category: "Puzzle" },
    pizza: { icon: "assets/prepara-la-pizza-icon.png", category: "Cucina" },
    balloons: { icon: "assets/scoppia-i-palloncini-icon.png", category: "Arcade" },
    aquarium: { icon: "assets/acquario-magico-icon.png", category: "Relax" },
    undercover: { icon: "assets/undercover-icon.png", category: "Party" },
    "block-blast": { icon: "assets/block-blast-icon.png", category: "Puzzle" },
    tetris: { icon: "assets/tetris-icon.png", category: "Puzzle" },
    wordle: { icon: "assets/wordle-icon.png", category: "Parole" },
    "reaction-time": { icon: "assets/reaction-time-icon.png", category: "Arcade" },
    "color-match": { icon: "assets/color-match-icon.png", category: "Reflex" },
    hangman: { icon: "assets/impiccato-icon.png", category: "Parole" },
    snake: { icon: "assets/snake-icon.png", category: "Arcade" },
    "sliding-puzzle": { icon: "assets/sliding-puzzle-icon.png", category: "Puzzle" },
    "number-merge": { icon: "assets/number-merge-icon.png", category: "Puzzle" },
    "water-sort": { icon: "assets/water-sort-icon.png", category: "Puzzle" },
    darts: { icon: "assets/darts-icon.png", category: "Arcade" },
    "polizia-ladri": { icon: "assets/polizia-ladri-icon.png", category: "Party" },
    "indovina-chi": { icon: "assets/indovina-chi-icon.png", category: "Party" },
    "gatto-topo": { icon: "assets/gatto-topo-icon.png", category: "Arcade" }
};
...
function normalizePublicGame(game) {
    ...
    const declaredStatus = CUGINIPARTY_PUBLIC_STATUSES.includes(rawStatus) ? rawStatus : "available";
    // coming_soon può avere enabled=false per compatibilità legacy: non è manutenzione.
    const status = declaredStatus === "coming_soon" || declaredStatus === "maintenance"
        ? declaredStatus
        : (game.enabled === false ? "maintenance" : "available");
    const description = String(game.description || "").trim()
        ? game.description
        : (String(fallback.description || "").trim() ? fallback.description : (CUGINIPARTY_DEFAULT_DESCRIPTIONS[gameId] || ""));
    const displayMeta = CUGINIPARTY_DEFAULT_DISPLAY_META[gameId] || null;
    const storedIcon = String(game.icon || "").trim();
    const legacyPlaceholderIcon = /^(?:◉|●|PL|P\s*L)$/i.test(storedIcon);
    const icon = displayMeta?.icon || storedIcon || game.icon;
    const category = String(game.category || "").trim()
        ? game.category
        : (displayMeta?.category || game.category);
    return {
        ...fallback,
        ...game,
...
function effectivePublicGameStatus(game) {
    const declaredStatus = CUGINIPARTY_PUBLIC_STATUSES.includes(game.status)
        ? game.status
        : "available";
    ...
    // Compatibilità con vecchi record che bloccavano il gioco solo con enabled=false.
    if (game.enabled === false) return "maintenance";
    return "available";
}

function arePublicGamesGloballyLocked() {
    return cuginiPartyAdminSettings?.global_games_locked === true;
}
...
function capturePublicStaticGameCards() {
    ...
    Array.from(grid.querySelectorAll(":scope > [data-game-id]:not(.dynamic-game-card)"))
        .forEach((card, index) => {
            ...
            const iconContainer = card.querySelector(".game-card-icon");
            const displayMeta = CUGINIPARTY_DEFAULT_DISPLAY_META[gameId];
            if (iconContainer && displayMeta?.icon) setPublicGameIcon(iconContainer, displayMeta.icon);
...
function updatePublicCard(card, game) {
    ...
    const displayMeta = CUGINIPARTY_DEFAULT_DISPLAY_META[String(game.game_id || "").toLowerCase()];
    if (displayMeta?.icon) {
        const iconContainer = card.querySelector(".game-card-icon");
        if (iconContainer) setPublicGameIcon(iconContainer, displayMeta.icon);
    }
...
function applyGameLinkState(link, game) {
    const status = effectivePublicGameStatus(game);
    const disabled = arePublicGamesGloballyLocked() || game.visible === false || status !== "available";
...
function createPublicDynamicGameCard(game) {
    ...
    setPublicGameIcon(icon, game.icon || "🎮");
...
function setPublicGameIcon(container, value) {
    if (!container) return;
    const icon = String(value || "🎮").trim() || "🎮";
    if (/^(?:\.\/)?assets\/[a-z0-9_./-]+\.(?:svg|png|webp)$/i.test(icon)) {
        const image = document.createElement("img");
        image.src = icon.replace(/^\.\//, "");
        image.alt = "";
...
function renderGameLinkStates() {
    ...
    const grid = document.querySelector(".games-grid");
    if (grid) {
        const order = new Map(cuginiPartyPublicAdminV2Records.map((game, index) => [game.game_id, index]));
        Array.from(grid.children).sort((a, b) => {
            const left = order.has(a.dataset.gameId) ? order.get(a.dataset.gameId) : Number.MAX_SAFE_INTEGER;
            const right = order.has(b.dataset.gameId) ? order.get(b.dataset.gameId) : Number.MAX_SAFE_INTEGER;
...
function showGameBlockedOverlay(game) {
    ...
    const settings = arePublicGamesGloballyLocked()
        ? {
            ...(game || getPublicGameRecord(current) || {}),
            game_id: current,
            visible: true,
            enabled: false,
            status: "maintenance",
            maintenance_message: "Tutti i giochi sono temporaneamente bloccati dall'amministratore."
        }
        : (game || getPublicGameRecord(current));
    ...
    const status = arePublicGamesGloballyLocked()
        ? "Giochi temporaneamente bloccati"
        : settings.visible === false ? "Gioco non disponibile" : effectivePublicGameStatus(settings) === "coming_soon" ? "Gioco in arrivo" : "Gioco in manutenzione";
    ...
    if (icon) icon.textContent = arePublicGamesGloballyLocked() ? "⛔" : settings.visible === false ? "🙈" : effectivePublicGameStatus(settings) === "coming_soon" ? "🚧" : "🔧";
...
function scheduleNextCuginiPartyGameAutoRelease() {
    if (cuginiPartyGameReleaseTimer) {
        clearTimeout(cuginiPartyGameReleaseTimer);
        cuginiPartyGameReleaseTimer = null;
    }

    const now = Date.now();
    const nextRelease = cuginiPartyPublicAdminV2Records
        .filter(game =>
            game.status === "coming_soon" &&
            game.auto_release &&
            game.scheduled_release_at
        )
        .map(game => new Date(game.scheduled_release_at).getTime())
        .filter(timestamp => Number.isFinite(timestamp) && timestamp > now)
        .sort((a, b) => a - b)[0];

    if (!nextRelease) return;

    // setTimeout ha un limite pratico di circa 24,8 giorni: per date più lontane
    // ricontrolliamo periodicamente senza anticipare il rilascio.
    const delay = Math.min(Math.max(0, nextRelease - now + 25), 2147483000);
    cuginiPartyGameReleaseTimer = setTimeout(() => {
        cuginiPartyGameReleaseTimer = null;
        renderGameLinkStates();
        const current = getCurrentGameId();
        if (current) showGameBlockedOverlay(getPublicGameRecord(current));
        scheduleNextCuginiPartyGameAutoRelease();
    }, delay);
}

function cuginiPartyAdditionOrderValue(game) {
    const index = CUGINIPARTY_ADDITION_ORDER.indexOf(String(game?.game_id || "").toLowerCase());
    if (index >= 0) return index;
    const created = game?.created_at ? new Date(game.created_at).getTime() : Number.MAX_SAFE_INTEGER;
    return CUGINIPARTY_ADDITION_ORDER.length + (Number.isFinite(created) ? created / 1e13 : 999999);
}

function applyCuginiPartyGameSettings(games) {
    cuginiPartyAdminGames = Array.isArray(games) ? games : [];
    cuginiPartyPublicAdminV2Records = cuginiPartyAdminGames
        .map(normalizePublicGame)
        .sort((a, b) => cuginiPartyAdditionOrderValue(a) - cuginiPartyAdditionOrderValue(b) || String(a.game_id).localeCompare(String(b.game_id), "it"));
    renderGameLinkStates();
    const current = getCurrentGameId();
    if (current) showGameBlockedOverlay(getPublicGameRecord(current));
...
async function loadCuginiPartyAdminGames() {
    ...
    try {
        const result = await window.supabaseClient.from("site_games").select("*").order("created_at", { ascending: true }).order("game_id", { ascending: true });
        if (result.error) throw result.error;
        const records = (result.data || []).map(normalizePublicGame);
        const knownIds = new Set(records.map(game => game.game_id));
        const staticGames = getStaticPublicGames().filter(game => !knownIds.has(game.game_id));
        const allGames = records.concat(staticGames);
        applyCuginiPartyGameSettings(allGames);
        return allGames;
    } catch (error) {
        console.warn("CuginiParty site_games non disponibile; uso il catalogo statico.", error.message || error);
        const staticGames = getStaticPublicGames();
        applyCuginiPartyGameSettings(staticGames);
        return staticGames;
    }
}

function getStaticPublicGames() {
    const ids = Array.from(document.querySelectorAll("[data-game-id]"))
        .map(card => String(card.dataset.gameId || "").trim().toLowerCase())
        .filter(Boolean);
    const currentGameId = getCurrentGameId();
    if (currentGameId && !ids.includes(currentGameId)) ids.push(currentGameId);
    return ids.map(gameId => normalizePublicGame({ game_id: gameId, enabled: true, visible: true, status: "available" }));
}
...
document.addEventListener("keydown", event => {
    const globalOverlay = document.getElementById("cuginiparty-admin-global-overlay");
    const gameOverlay = document.getElementById("cuginiparty-game-blocked-overlay");
    const isBlocked = globalOverlay?.classList.contains("visible") || gameOverlay?.classList.contains("visible");
    if (isBlocked && !event.target.closest("button, input, select, textarea, a")) {
        event.preventDefault();
        event.stopImmediatePropagation();
    }
}, true);

async function ensureCuginiPartySupabaseClient() {
...
async function bootCuginiPartyAdminControl() {
    await ensureCuginiPartySupabaseClient();
    await initCuginiPartyAdminControl();
}
