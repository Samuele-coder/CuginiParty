...
const CUGINIPARTY_ADDITION_ORDER = [
    ...
    "cube-rush", "polizia-ladri", "indovina-chi"
];
...
const CUGINIPARTY_DEFAULT_DISPLAY_META = {
    snake: { icon: "●", category: "Arcade" },
    "sliding-puzzle": { icon: "▦", category: "Puzzle" },
    "number-merge": { icon: "2²", category: "Puzzle" },
    "water-sort": { icon: "🧪", category: "Puzzle" },
    darts: { icon: "🎯", category: "Arcade" },
    "polizia-ladri": { icon: "assets/polizia-ladri-icon.svg", category: "Party" },
    "indovina-chi": { icon: "assets/indovina-chi-icon.svg", category: "Party" }
};
...
function normalizePublicGame(game) {
    ...
    const icon = storedIcon && !(legacyPlaceholderIcon && displayMeta?.icon)
        ? storedIcon
        : (displayMeta?.icon || game.icon);
...
function updatePublicCard(card, game) {
    ...
    // Il catalogo dinamico può aggiornare una card già presente nell'HTML.
    // Per Polizia e Ladri forza l'asset vettoriale anche in quel caso, così
    // un vecchio segnaposto "PL" non rimane visibile dopo il caricamento.
    if (
        String(game.game_id || "").toLowerCase() === "polizia-ladri" &&
        /^assets\/polizia-ladri-icon\.svg$/i.test(String(game.icon || ""))
    ) {
        const iconContainer = card.querySelector(".game-card-icon");
        if (iconContainer) setPublicGameIcon(iconContainer, game.icon);
    }
